...
// Adicionar a quantidade de tropas deseja para unidade desejada e o tipo da aldeia
...
function geraTropas() {
    objetoTropas = [
        ...
        { nomeUnidade: "spy", recrutarAtt: 0, recrutarDef: 0, cssClassSelector: classEnum.explorador },
        { nomeUnidade: "light", recrutarAtt: 1, recrutarDef: 0, cssClassSelector: classEnum.cavalariaLeve },
        ...
        { nomeUnidade: "ram", recrutarAtt: 1, recrutarDef: 0, cssClassSelector: classEnum.ariete },
        { nomeUnidade: "catapult", recrutarAtt: 1, recrutarDef: 0, cssClassSelector: classEnum.catapulta }
    ];
}
// ======== CONFIGURAÇÃO ========
...
$(document).ready(setInterval(function () {
    ...
    if (ataque.includes(village)) {
        ...
        console.log("Verificando Tropas [" + village + "] - Aldeia não configurada. Não recrutando!");
    }

    var retorno = false;
    objetoTropas.forEach(element => {
        var response = validarPreencher(element, village);
        if (!retorno) {
            retorno = response;
        }
    });
